...
from doclayout_yolo.cfg import TASK2DATA, get_cfg, get_save_dir
from doclayout_yolo.hub.utils import HUB_WEB_ROOT
from doclayout_yolo.nn.tasks import attempt_load_one_weight, guess_model_task, nn, yaml_model_load
from doclayout_yolo.utils import ASSETS, DEFAULT_CFG_DICT, LOGGER, RANK, SETTINGS, callbacks, checks, emojis, yaml_load
...
class Model(nn.Module):
    ...
    def __init__(
        ...
        # Check if Ultralytics HUB model from https://hub.doclayout_yolo.com
    ...
    def _get_hub_session(model: str):
        ...
        from doclayout_yolo.hub.session import HUBTrainingSession
    ...
    def is_hub_model(model: str) -> bool:
        ...
        return any(
            (
                model.startswith(f"{HUB_WEB_ROOT}/models/"),  # i.e. https://hub.doclayout_yolo.com/models/MODEL_ID
    ...
    def _load(self, weights: str, task=None) -> None:
        ...
        PATCHED: Safe loading without assuming model.args exists.
        """
        ...
        if Path(weights).suffix == ".pt":
            # Эта функция теперь безопасно загружает веса (благодаря патчу в tasks.py)
            self.model, self.ckpt = attempt_load_one_weight(weights)
            
            # --- START SAFE PATCH ---
            self.task = task or "detect"
            
            # 1. Безопасное получение imgsz
            # Проверяем: есть ли у модели args? Если да, то какого они типа?
            current_imgsz = 1024 # Значение по умолчанию
            
            if hasattr(self.model, 'args') and self.model.args is not None:
                if isinstance(self.model.args, dict):
                    current_imgsz = self.model.args.get('imgsz', 1024)
                elif hasattr(self.model.args, 'imgsz'):
                    current_imgsz = self.model.args.imgsz
            
            # 2. Формируем overrides
            self.overrides = {
                "model": weights,
                "task": self.task,
                "imgsz": current_imgsz,
                "data": None,  # КРИТИЧЕСКИ ВАЖНО
                "single_cls": False
            }
            
            # 3. Безопасное обновление args внутри модели (если они есть)
            if hasattr(self.model, 'args') and self.model.args is not None:
                if isinstance(self.model.args, dict):
                    self.model.args['data'] = None
                    self.model.args['task'] = self.task
                elif hasattr(self.model.args, 'data'):
                    self.model.args.data = None
                    self.model.args.task = self.task
                
            self.ckpt_path = getattr(self.model, 'pt_path', weights)
            # --- END SAFE PATCH ---
            ...
            self.overrides = {"model": weights, "task": self.task, "data": None}
    ...
    def save(self, filename: Union[str, Path] = "saved_model.pt", use_dill=True) -> None:
        ...
        updates = {
            ...
            "version": __version__,
            "license": "AGPL-3.0 License (https://doclayout_yolo.com/license)",
    ...
    def benchmark(
        ...
        **kwargs,
        ...
        """
        self._check_is_pytorch_model()
        from doclayout_yolo.utils.benchmarks import benchmark
    ...
    def tune(
        ...
        **kwargs,
        ...
        self._check_is_pytorch_model()
        if use_ray:
            from doclayout_yolo.utils.tuner import run_ray_tune
    ...
    def names(self) -> list:
        ...
        """
        from doclayout_yolo.nn.autobackend import check_class_names
